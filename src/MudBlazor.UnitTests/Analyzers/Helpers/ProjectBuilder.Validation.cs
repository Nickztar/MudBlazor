...
public sealed partial class ProjectBuilder
{
    ...
    private static void VerifyDiagnosticLocation(IList<DiagnosticAnalyzer> analyzers, Diagnostic diagnostic, Location actual, in DiagnosticResultLocation expected)
    {
        ...
        var correctLocation = string.Equals(actualSpan.Path, expected.Path, StringComparison.Ordinal) || (actualSpan.Path is not null && actualSpan.Path.StartsWith("Test", StringComparison.Ordinal) && expected.Path.EndsWith(".cs", StringComparison.Ordinal));
        correctLocation.Should().BeTrue(string.Format(CultureInfo.InvariantCulture, "Expected diagnostic to be in file \"{0}\" was actually in file \"{1}\"\r\n\r\nDiagnostic:\r\n    {2}\r\n", expected.Path, actualSpan.Path, FormatDiagnostics(analyzers, diagnostic)));
    ...
    private async Task VerifyFix(IList<DiagnosticAnalyzer> analyzers, CodeFixProvider codeFixProvider, string newSource, int? codeFixIndex)
    {
        ...
        // Note: Ignoring line endings in fluentassertions?
        // Assert.Equal(newSource, actual, ignoreLineEndingDifferences: true);
        newSource.ReplaceLineEndings().Should().BeEquivalentTo(actual.ReplaceLineEndings());
    }
